...
def get_sat_data():
    ...
    data_path = osp.join("./datasetes", "satimage", "sat.trn")
    with open(data_path) as f:
        rows = [row.strip().split(',') for row in f.readlines()]
    n_datas = len(rows)
    X_train = np.zeros((n_datas, 36), dtype=np.int32)
    y_train = np.zeros(n_datas, dtype=np.int32)
    for i, row in enumerate(rows):
        X_train[i, :] = list(map(int, row[0:0 + 36]))
        y_train[i] = int(row[-1])


    data_path = osp.join("./datasetes", "satimage", "sat.tst")
    with open(data_path) as f:
        rows = [row.strip().split(',') for row in f.readlines()]
    n_datas = len(rows)
    X_test = np.zeros((n_datas, 36), dtype=np.int32)
    y_test = np.zeros(n_datas, dtype=np.int32)
    for i, row in enumerate(rows):
        X_test[i, :] = list(map(int, row[0:0 + 36]))
        y_test[i] = int(row[-1])
